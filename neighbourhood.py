...
class neighbourhood:
	...
	def __init__(self, householdList):
		...
		#Add PV to houses:
		for i in range(0, len(householdList)):
			if i < (round(len(householdList)*(config.penetrationPV/100))):
				pvList[i] = 1
		...
		for i in range(0, len(householdList)):
			if i < (round(len(householdList)*(config.penetrationInductioncooking/100))):
		...
		i = 0
		while i < (round(len(householdList)*(config.penetrationCHP/100))) - (round(len(householdList)*(config.penetrationPV/100))):
			...
			if householdList[j].hasCHP == False and pvList[j] == 0: # First supply houses without PV
				...
				i = i + 1
		if (round(len(householdList)*(config.penetrationPV/100))) > (round(len(householdList)*(config.penetrationCHP/100))): # If there are too much CHPs compared to PV, add some more CHPS
			while i < (round(len(householdList)*(config.penetrationCHP/100))):
				j = random.randint(0,len(householdList)-1)
		...
		# Add heat pumps
		i = 0
		while i < (round(len(householdList)*(config.penetrationHeatPump/100))):
			j = random.randint(0,len(householdList)-1)
		...
		#Now add batteries
		i = 0
		while i < (round(len(householdList)*(config.penetrationBattery/100))):
			j = random.randint(0,len(householdList)-1)
		...
		for i in range(0, len(drivingDistance)):
			if i < (round(len(householdList)*(config.penetrationEV/100))+round(len(householdList)*(config.penetrationPHEV/100))):
				...
				while added == False:
					if householdList[j].Persons[0].DistanceToWork == drivingDistance[i]:
						if householdList[j].hasEV == False:
							if i < (round(len(householdList)*(config.penetrationEV/100))):
								householdList[j].Devices["ElectricalVehicle"].BufferCapacity = config.capacityEV
								householdList[j].Devices["ElectricalVehicle"].Consumption = config.powerEV
							else:
								householdList[j].Devices["ElectricalVehicle"].BufferCapacity = config.capacityPHEV
								householdList[j].Devices["ElectricalVehicle"].Consumption = config.powerPHEV
		...
		for i in range(0,len(householdList)):
			...
			if pvList[i] == 1:
				...
				area = round( (householdList[i].ConsumptionYearly / config.PVProductionPerYear) * 1.6) #average panel is 1.6m2
			...
			if batteryList[i] == 1:
				...
				if householdList[i].hasEV:
					...
					householdList[i].House.addBattery(config.capacityBatteryLarge, config.powerBatteryLarge) #Let's give it a nice battery!
				else:
					#NO EV, just some peak shaving:
					if(householdList[i].ConsumptionYearly > 2500):
						householdList[i].House.addBattery(config.capacityBatteryMedium, config.powerBatteryMedium)
					else:
						householdList[i].House.addBattery(config.capacityBatterySmall, config.powerBatterySmall)
